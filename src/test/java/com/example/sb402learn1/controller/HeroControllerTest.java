...
class HeroControllerTest {
    ...
    void getHero() throws Exception {
        Hero hero = Hero.builder().id(UUID.randomUUID()).name("name100").title("title100").birthday(LocalDateTime.now()).apple("apple100").build();
        when(heroService.getHero()).thenReturn(hero);
        mockMvc.perform(get("/hero/hero"))
